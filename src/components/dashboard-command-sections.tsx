...
import { EmptyState, InlinePill, Panel, PrimaryButton, RiskBadge, SecondaryButton } from "@/components/ui";
...
const AlertTableRow = memo(function AlertTableRow({ alert, index }: { alert: RiskAnalysis; index: number }) {
  return (
...
const ReportTableRow = memo(function ReportTableRow({ report }: { report: CommunityReport }) {
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-white/[0.04]">
...
export default function DashboardCommandSections() {
  ...
  return (
    <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[680px] text-left text-sm">
                <thead className="border-b border-black/5 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-400">
                  <tr>
                    <th className="px-4 py-3">Location</th><th className="px-4 py-3">Threat</th><th className="px-4 py-3">Risk</th><th className="px-4 py-3">Confidence</th><th className="px-4 py-3">Updated</th><th className="px-4 py-3 text-right">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-black/5 dark:divide-white/10">{alerts.map((alert, index) => <AlertTableRow key={`${alert.location}-${index}`} alert={alert} index={index} />)}</tbody>
              </table>
            </div>
          )}
        </Panel>
        ...
        <Panel className="overflow-hidden p-0">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[560px] text-left text-sm">
              <thead className="border-b border-black/5 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-400">
                <tr><th className="px-4 py-3">Type</th><th className="px-4 py-3">Location</th><th className="px-4 py-3">Severity</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Time</th></tr>
              </thead>
              <tbody className="divide-y divide-black/5 dark:divide-white/10">
                {reports.filter((report) => report.severity === "High" || report.severity === "Critical").concat(reports.filter((report) => report.severity !== "High" && report.severity !== "Critical")).slice(0, 8).map((report) => <ReportTableRow key={report.id ?? `${report.location}-${report.description}`} report={report} />)}
              </tbody>
            </table>
          </div>
        </Panel>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[360px_1fr]">
        <Panel>
          <div className="flex items-center gap-2"><MapPinned size={18} className="text-emergency-green" /><h2 className="font-semibold text-slate-950 dark:text-white">Affected areas</h2></div>
          ...
          <div className="mt-4 space-y-2">{[...new Set([...alerts.map((alert) => alert.location), ...reports.map((report) => report.location)])].slice(0, 6).map((location) => <div key={location} className="rounded-xl border border-black/5 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">{location}</div>)}</div>
        </Panel>
                ...
                <div className="space-y-3">
                  {buildResponseCards(alerts, reports, column).map((item) => (
                    <div key={item.id} className="rounded-xl border border-black/5 bg-white p-3 text-sm shadow-soft dark:border-white/10 dark:bg-slate-950/60">
                      <div className="flex items-start justify-between gap-2"><p className="font-semibold text-slate-950 dark:text-white">{item.location}</p><RiskBadge level={item.severity} /></div>
                      ...
                      <div className="mt-3 flex gap-2"><PrimaryButton disabled className="px-3 py-1.5">Update soon</PrimaryButton></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-2">
        <Panel className="p-0">
          <div className="border-b border-black/5 px-5 py-4 dark:border-white/10"><h2 className="font-semibold text-slate-950 dark:text-white">Responder units</h2></div>
          <div className="divide-y divide-black/5 dark:divide-white/10">{responderUnits.map((unit) => <div key={unit.id} className="grid gap-3 px-5 py-3 md:grid-cols-[1fr_auto] md:items-center"><div><p className="text-sm font-semibold text-slate-950 dark:text-white">{unit.name}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{unit.role} · {unit.location} · {unit.lastSeen}</p></div><InlinePill className={statusTone(unit.status)}>{unit.status}</InlinePill></div>)}</div>
        </Panel>
        <Panel className="p-0">
          <div className="border-b border-black/5 px-5 py-4 dark:border-white/10"><h2 className="font-semibold text-slate-950 dark:text-white">Notification delivery</h2></div>
          <div className="divide-y divide-black/5 dark:divide-white/10">{notificationQueue.map((item) => <div key={item.id} className="grid gap-3 px-5 py-3 md:grid-cols-[1fr_auto] md:items-center"><div><p className="text-sm font-semibold text-slate-950 dark:text-white">{item.channel}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{item.target} · {item.time}</p></div><InlinePill className={statusTone(item.status)}>{item.status}</InlinePill></div>)}</div>
        </Panel>
      </section>
    </>
  );
